...
template<typename Object, typename F,int index=0>
inline auto for_each_element(Object&object,F&& callback);                 
...
template<typename Object, typename F, int index>
inline auto for_each_element(Object&object,F&&callback)                              //对于每一个元素进行遍历
{
	callback(std::get<index>(object),index);
	if constexpr(index+1<std::tuple_size<Object>::value)
		for_each_element<Object, F, index+1>(object, std::forward<F>(callback));
}
